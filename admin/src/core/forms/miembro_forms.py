...
from src.core.forms.validaciones import (
    LimiteDeArchivo,
    Unico,
    validar_digitos,
    TipoDeArchivo,
...
class InfoMiembroForm(FlaskForm):
    ...
    dni = IntegerField(
        ...
        validators=[
            ...
            NumberRange(
                min=1000000,
                max=99999999,
                message="El DNI debe estar entre 1000000 y 99999999.",
            ),
        ],
    )
    email = StringField(
    ...
    telefono_contacto_emergencia = IntegerField(
    ...
    condicion_id = SelectField(
        "Condición de Trabajo*",
        validators=[DataRequired("Ingrese la condición de trabajo.")],
    )
    profesion_id = SelectField(
        "Profesión*", validators=[DataRequired("Ingrese la profesión.")]
    )
    puesto_laboral_id = SelectField(
        "Puesto Laboral*", validators=[DataRequired("Ingrese el puesto laboral.")]
    )
    calle = StringField(
        "Calle*",
        validators=[
    ...
    numero = IntegerField(
        ...
        validators=[
            ...
            NumberRange(
                min=0,
                max=10000,
                message="El valor del número debe estar entre 0 y 10000.",
            ),
        ],
    )
    piso = IntegerField(
        ...
        validators=[
            Optional(),
            NumberRange(
                min=0, max=100, message="El valor del piso debe estar entre 0 y 100."
            ),
        ],
    )
    dpto = StringField(
...
class ArchivoMiembroForm(FlaskForm):
    ...
    tipo_de_documento_id = SelectField(
        "Tipo", coerce=int, validators=[DataRequired("Seleccione una opcion")]
    )
    submit = SubmitField("Guardar")
...
class EnlaceMiembroForm(FlaskForm):
    ...
    url = StringField(
        "Enlace", validators=[DataRequired("Ingrese el enlace."), url_valida]
    )
    tipo_de_documento_id = SelectField(
        "Tipo", validators=[DataRequired("Seleccione un opcion.")], coerce=int
    )
    submit = SubmitField("Guardar")
