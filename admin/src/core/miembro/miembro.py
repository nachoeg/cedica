...
class Miembro(db.Model):
    ...
    condicion_id = db.Column(
        db.Integer, db.ForeignKey("condicion_trabajo.id"), nullable=False
    )
    ...
    def to_dict(self):
        return {
            ...
            "puesto_laboral": (
                self.puesto_laboral.nombre if self.puesto_laboral else None
            ),
            "fecha_creacion": (
                self.fecha_creacion.strftime("%d-%m-%Y")
